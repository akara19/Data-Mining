<?php
error_reporting(0);
class Ppi_mahasiswa extends CI_Controller{
	...
	function __construct() {
		...
		$this->load->model('Ppi_mahasiswa_model');
	}
	// index untuk tabel ppi_mahasiswa berfungsi untuk menampilkan data pada grid
	function index(){
		$data['Type']="Data Mahasiswa";
		$data['Ppi_mahasiswa'] = $this->Ppi_mahasiswa_model->ppi_mahasiswa_view()->result();
		$this->template->load('dashboard','webadmin/Ppi_mahasiswa/Ppi_mahasiswa',$data);
	}
